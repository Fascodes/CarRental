...
INSERT INTO listings (car_id, user_id, price, status, title, body, created_at, last_active) VALUES
  (
    (SELECT id FROM cars WHERE vin = 'JT2BF22K1W0123001'),
    (SELECT id FROM users WHERE email = 'alice@dev.local'),
    120, 'ACTIVE', 'Toyota Corolla — reliable city car',
  ...
  (
    ...
    (SELECT id FROM users WHERE email = 'alice@dev.local'),
    250, 'ACTIVE', 'BMW 3 Series — premium comfort',
  ...
  (
    ...
    90, 'ACTIVE', 'Ford Focus — budget friendly',
